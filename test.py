...
def runTests(game):
    ...
    test(
        -10 <= game.getCurrentWind() <= 10, "wind should be a random value in [-10,10]"
    )
    ...
    test(
        proj.getX() == game.getCurrentPlayer().getX(),
        "Fired projectile should start at player X-position",
    )
    test(
        abs(proj.getY() - 10 / 2) < 0.01,
        "Fired projectile Y-position should start half the cannon size",
    )
    test(proj.isMoving() == True, "projectile should be moving")

    test(
        game.getCurrentPlayer().getAim() == (30, 31),
        "After firing, getAim() should give the latest (angle,velocity)-values",
    )

    proj.update(1.0)
    test(
        abs(proj.getX() + 63.1532124826824) < 0.01,
        "Projectile X-Position is {0:f}, should be -63.153212".format(proj.getX()),
    )
    test(
        abs(proj.getY() - 15.6) < 0.01,
        "Projectile X-Position is {0:f}, should be 15.6".format(proj.getY()),
    )
    ...
    test(
        abs(proj.getX() - 3.9637563106115907) < 0.01,
        "Projectile X-Position is {0:f}, should be -3.9637563106115907".format(
            proj.getX()
        ),
    )

    test(
        abs(players[1].projectileDistance(proj) - -78.03624368938841) < 0.01,
        "Projectile X-distance to player is {0:f}, should be -78.03624368938841".format(
            players[1].projectileDistance(proj)
        ),
    )
    test(
        abs(players[0].projectileDistance(proj) - 85.96375631061159) < 0.01,
        "Projectile X-distance to player is {0:f}, should be 85.96375631061159".format(
            players[0].projectileDistance(proj)
        ),
    )
    ...
    test(
        abs(proj.getX() - -86.84740597475547) < 0.01,
        "Projectile X-Position is {0:f}, should be -86.84740597475547".format(
            proj.getX()
        ),
    )
    test(
        abs(players[1].projectileDistance(proj) - -168.84740597475547) < 0.01,
        "Projectile X-distance to player is {0:f}, should be 168.84740597475547".format(
            players[1].projectileDistance(proj)
        ),
    )
    test(
        players[0].projectileDistance(proj) == 0,
        "Projectile X-distance to player is {0:f}, should be 0".format(
            players[0].projectileDistance(proj)
        ),
    )
    ...
    test(ticks == 61, "Incorrect tick-count")
    test(
        abs(proj.getX() - 68.2424059747553) < 0.01,
        "Projectile X-Position is {0:f}, should be 68.2424059747553".format(
            proj.getX()
        ),
    )
    ...
    if gameAtts > 5:
        print(
            "Your Game object has {} attributes. This isn't necessarily wrong, but 5 seems like a nice number.".format(
                gameAtts
            )
        )
        print(
            "Make sure you are not representing the same information in multiple attributes."
        )
    playerAtts = len(game.getCurrentPlayer().__dict__.items())
    if playerAtts > 8:
        print(
            "Your Player object has {} attributes. This isn't necessarily wrong, but it seems a bit high.".format(
                playerAtts
            )
        )
...
def run(src_path=None):
    ...
    if src_path == None:
        ...
        spec = importlib.util.spec_from_file_location(
            "gamemodel", src_path + "/gamemodel.py"
        )
